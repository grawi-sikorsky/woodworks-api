...
@Slf4j
@Component
public class TwoDrawerCabinGenerator implements CabinCuttingStrategy {
    ...
    public boolean supports(CabinType type) {
        return type == CabinType.TWO_DRAWER;
    }

    @Override
    public List<Item> generateItems(WorkRequest.CabinRequest cabinRequest) {
        log.info("Generating two drawer cabin items.");
        ...
        // Fronty Szuflad (2 równe)
        int drawerFrontHeight = (height - FRONT_CLEARANCE) / 2 - 2; // Simplified gap logic
        items.add(new Item("Front 1", width - FRONT_CLEARANCE, drawerFrontHeight, thickness, 1, PLYTA_MEBLOWA));
        items.add(new Item("Front 2", width - FRONT_CLEARANCE, drawerFrontHeight, thickness, 1, PLYTA_MEBLOWA));
