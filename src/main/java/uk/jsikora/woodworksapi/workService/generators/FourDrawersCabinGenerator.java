...
@Slf4j
@Component
public class FourDrawersCabinGenerator implements CabinCuttingStrategy {
    ...
    public boolean supports(CabinType type) {
        return type == CabinType.FOUR_DRAWERS;
    }

    @Override
    public List<Item> generateItems(WorkRequest.CabinRequest cabinRequest) {
        log.info("Generating four drawers cabin items.");
        ...
        // Fronty Szuflad (4 równe)
        int drawerFrontHeight = (height - FRONT_CLEARANCE) / 4 - 2; // Simplified gap logic
        for (int i = 0; i < 4; i++) {
